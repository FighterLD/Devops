...
def connect_remote_mongohost(remote_host='192.168.0.204', ssh_port=22):
    """设置远程mongo host and user
    """
    env.hosts = ['%s:%d' % (remote_host, ssh_port)]
    env.user = 'root'

def _remote_mongodump(host='localhost', port=27057, username='', password='', authendb='recruiting', db='', collection='', out='/data/mongo/data/'):
    ...
    outname = out/date.bson
    with cd(out):
        mongodump_cmd = 'mongodump --host %s --port %d --username %s --password %s --authenticationDatabase %s --db %s --collection %s --directoryperdb --out %s' % (host, port, username, password, authendb, db, collection, outname)
        run(mongodump_cmd)
